...
class ShortLongReward:
    """Reward calculation class for short-long trading environment.
    ...
    def __call__(
        ...
        action_map = {
            ...
            "LONG": (lambda: self.open_long_reward(price_info, current_holding), 1),
            "CloseLong": (lambda: self.close_long_reward(price_info), 0),
            "PLONG": (lambda: self.close_long_reward(price_info), 0),
            "OpenShort": (
                lambda: self.open_short_reward(price_info, current_holding),
                -1,
            ),
            "SHORT": (lambda: self.open_short_reward(price_info, current_holding), -1),
            "CloseShort": (lambda: self.close_short_reward(price_info), 0),
            "PSHORT": (lambda: self.close_short_reward(price_info), 0),
        }
